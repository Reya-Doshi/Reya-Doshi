...
def from_github(user: str, token: str | None, limit: int, exclude: set[str],
    ...
    try:
        ...
        print(f"  note: unable to fetch github repos for '{user}' ({e}), using profile language mix", file=sys.stderr)

    if not totals:
        # Fallback profile language mix for local generation / preview
        return f"{user} · language mix", [
            ("Python", 90.0),
            ("Java", 82.0),
            ("TypeScript", 75.0),
            ("C++", 65.0),
